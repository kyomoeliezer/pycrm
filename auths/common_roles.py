...
def _can_review_request(user):
    if user.role:
        if user.role.perm:
            if user.role.perm.filter(perm_name__iexact='review').exists():
                return True
    return False
...
def _can_approve_request(user):
    if user.role:
        if user.role.perm:
            if user.role.perm.filter(perm_name__iexact='approve').exists():
                return True
    return False
...
def _action_on_request(user):
    if user.role:
        if user.role.perm:
            if user.role.perm.filter(perm_name__in=['approve','review']).exists():
                return True
    return False
